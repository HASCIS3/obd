<section>
    ...
    <div class="mt-6 flex items-center gap-6">
        <div class="shrink-0">
            @if($user->photo_url)
                <img class="h-20 w-20 object-cover rounded-full ring-2 ring-primary-500" src="{{ $user->photo_url }}" alt="{{ $user->name }}">
            @else
        ...
        <div>
            <form method="post" action="{{ route('profile.photo.update') }}" enctype="multipart/form-data" class="flex items-center gap-3">
                @csrf
                <label class="cursor-pointer bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-md text-sm font-medium transition">
                    Changer la photo
            ...
            @if (session('status') === 'photo-updated')
                <p class="text-sm text-green-600 mt-2">Photo mise a jour.</p>
            @endif
            @if (session('status') === 'photo-deleted')
                <p class="text-sm text-green-600 mt-2">Photo supprimee.</p>
            @endif
        </div>
    </div>
